...
1. 動画を開くとキャンバス下にタイムラインが出ます。スライダーや `←` / `→` で
   ...
   範囲の編集は止まります)。現在のフレーム番号と時刻はウィンドウタイトルにも出ます
...
3. 「自動検出」を実行すると範囲を指定するダイアログが出ます。開始・終了フレーム
   (既定は先頭から末尾まで) と検出間隔を決めると、その範囲を走査し、
   検出 1 件ごとに [検出フレーム, 次の検出フレームの直前] の区間つき範囲を追加します
   (フレームごとに独立で、別フレームの検出結果には影響されません。
   セグメンテーションモデルは輪郭の多角形、検出のみのモデルは矩形になります)。
   指定した範囲に掛かる前回の自動検出結果は、今回の結果で置き換えられます
   (手描きのペン・矩形は残ります)
4. 検出間隔を増やすと、間引いて検出して高速化できます
   (漏れやすくなるため、動きの少ない動画向け)
5. `Ctrl+S` / `Cmd+S` で書き出します。先にフォーマット (H.264 / H.265 / 無圧縮 AVI)・
   解像度・品質 (CRF) を選ぶダイアログが出ます。保存先は同じ場所の `名前_mc.mp4`
   (無圧縮のみ `名前_mc.avi`) で、音声はそのまま引き継がれます
   (mp4 に載らない音声形式は AAC、無圧縮 AVI では PCM へ変換)。
   無圧縮は出力サイズが事前に読めるため、書き出し前に必要量と保存先の空き容量を
   毎回表示します (足りない場合はそのまま中止、足りていれば続行するか選べます)
