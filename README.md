...
1. 動画を開くとキャンバス下にタイムラインが出ます。スライダーや `←` / `→` で
   フレームを移動できます
2. 範囲には適用区間 (開始/終了フレーム) があります。スライダー上の区間バーに
   全範囲の区間が帯で表示され、範囲を選択すると濃い帯になります。
   濃い帯の両端をドラッグして開始/終了フレームを変更できます
   (手動で追加した範囲の区間は、最初は追加したフレームのみです)。
   薄い帯をクリックするとその範囲を選択できます。選択中の範囲は
   フレームを移動しても選択と表示が保たれます
...
4. タイムラインの「検出間隔」を増やすと、間引いて検出して高速化できます
